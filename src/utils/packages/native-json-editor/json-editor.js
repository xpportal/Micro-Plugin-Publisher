class JSON_Editor extends HTMLElement {
	...
	format_string(input) {
		// Only handle HTML display escaping, don't modify JSON escapes
		return `<span part="string"><span part="string_quotes">\"</span>${input
			.replace(/&/g, "&amp;")
			.replace(/</g, "&lt;")
			.replace(/>/g, "&gt;")
	...
	get raw_string() {
		...
		let content = this.editor.innerText?.replaceAll('\xa0', '') || '';
		
		// Process the content to escape string values
		try {
			// First parse to get the structure
			const parsed = JSON.parse(content);
			
			// Recursively process the object to escape string values
			...
			// Process and re-stringify with proper escaping
			const escaped = escapeStrings(parsed);
			const stringified = JSON.stringify(escaped, null, 2);
	
			// Clean up any double escapes that might have been created
			return stringified	
		} catch (e) {
			// If parsing fails, return the original content
			console.warn('Failed to process JSON content:', e);
			return content;
		}
	}		
